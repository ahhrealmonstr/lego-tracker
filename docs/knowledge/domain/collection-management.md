...
The collection module in `src/domain/collection.ts` manages owned LEGO items — creation, updates, and summary aggregation.
...
Called by `addItem` in `App` component in `src/api/main.tsx`.
